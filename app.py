...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:@localhost/gdb_cash_db'
...
def get_logs():
    """Mengambil log aktivitas dengan pagination"""
...
def get_all_program_kerja():
    # Ambil parameter periode dari query string (opsional)
...
def delete_user(id_user):
    ...
    if user.username == 'admin':
        return jsonify({'status': 'error', 'message': 'Tidak dapat menghapus user admin'}), 400
    
    # Soft delete: set deleted_at dengan waktu sekarang
    ...
    log_entry = LogAktivitas(
        ...
        deskripsi=f'User {user.username} dinonaktifkan (soft delete)',
        ip_address=request.remote_addr
    )
    db.session.add(log_entry)
    db.session.commit()
...
def restore_user(id_user):
    ...
    if user.deleted_at is None:
        return jsonify({'status': 'error', 'message': 'User masih aktif'}), 400
    ...
    log_entry = LogAktivitas(
        id_pengguna=user.id_user,
        aktivitas='Restore User',
        deskripsi=f'User {user.username} diaktifkan kembali',
        ip_address=request.remote_addr
    )
    db.session.add(log_entry)
    db.session.commit()
    
    return jsonify({'status': 'success', 'message': 'User berhasil diaktifkan kembali'})

# ==================== ENDPOINT CETAK LAPORAN PDF ====================
...
def generate_laporan_pdf(id_program):
    ...
    story.append(Paragraph(f"LAPORAN KEUANGAN PROGRAM KERJA", title_style))
    ...
    ringkasan_data = [
        ['Total Pemasukan', f"Rp {total_masuk:,.0f}"],
        ['Total Pengeluaran', f"Rp {total_keluar:,.0f}"],
...
def get_dashboard_stats():
    ...
    # Hitung total pemasukan (tanpa filter periode dulu)
    ...
    # Hitung program aktif berdasarkan periode aktif
    if tahun_aktif:
...
def get_all_users():
    # Hanya ambil user yang belum dihapus (deleted_at IS NULL)
...
def reset_password_user(id_user):
    ...
    db.session.commit()
    return jsonify({'status': 'success', 'message': 'Password berhasil direset menjadi password123'})

# ==================== ENDPOINT PERIODE AKTIF ====================
...
def save_periode_aktif_status():
    data = request.get_json()
    tahun_sekarang = data.get('tahun_sekarang', False)
    tahun_depan = data.get('tahun_depan', False)
    
    current_year = datetime.now().year
    next_year = current_year + 1
    
    # Update atau insert periode tahun sekarang
    periode = PeriodeAktif.query.filter_by(nama_periode='Tahun Sekarang').first()
    if periode:
        periode.status_periode = 'Aktif' if tahun_sekarang else 'Nonaktif'
        periode.tahun = current_year
    else:
        periode = PeriodeAktif(
            nama_periode='Tahun Sekarang',
            tahun=current_year,
            status_periode='Aktif' if tahun_sekarang else 'Nonaktif'
        )
        db.session.add(periode)
    
    # Update atau insert periode tahun depan
    periode2 = PeriodeAktif.query.filter_by(nama_periode='Tahun Depan').first()
    if periode2:
        periode2.status_periode = 'Aktif' if tahun_depan else 'Nonaktif'
        periode2.tahun = next_year
    else:
        periode2 = PeriodeAktif(
            nama_periode='Tahun Depan',
            tahun=next_year,
            status_periode='Aktif' if tahun_depan else 'Nonaktif'
        )
        db.session.add(periode2)
    
    db.session.commit()
    return jsonify({'status': 'success', 'message': 'Periode aktif berhasil disimpan'})
...
def create_transaksi():
    """Menambah transaksi baru (dengan upload file)"""
    ...
    return jsonify({'status': 'success', 'message': 'Transaksi berhasil ditambahkan', 'id': new_transaksi.id_transaksi})
...
def update_transaksi(id_transaksi):
    ...
    transaksi.id_kategori = data.get('id_kategori', transaksi.id_kategori)
    ...
    transaksi.bukti_file = data.get('bukti_file', transaksi.bukti_file)
    transaksi.status_validasi = data.get('status_validasi', transaksi.status_validasi)
    db.session.commit()
    return jsonify({'status': 'success', 'message': 'Transaksi berhasil diupdate'})
...
def delete_transaksi(id_transaksi):
    ...
    db.session.commit()
    return jsonify({'status': 'success', 'message': 'Transaksi berhasil dihapus'})

# ==================== ENDPOINT RIWAYAT PROGRAM KERJA SELESAI ====================

@app.route('/api/program-kerja/selesai', methods=['GET'])
def get_program_kerja_selesai():
    try:
        # Ambil semua program kerja dengan status 'Selesai'
        # Urutkan dari yang terbaru dibuat (created_at DESC)
        ...
        print(f"Error di /api/program-kerja/selesai: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Gagal mengambil data program kerja selesai',
            'error': str(e)
        }), 500

# ==================== ENDPOINT PENGADAAN MENUNGGU KONFIRMASI ====================

@app.route('/api/pengajuan/menunggu', methods=['GET'])
def get_pengajuan_menunggu_list():
    try:
        # Ambil semua pengajuan dengan status 'Menunggu'
...
# ==================== ENDPOINT KONFIRMASI TRANSAKSI ====================

@app.route('/api/pengajuan/<int:id_pengajuan>/konfirmasi', methods=['POST'])
def proses_konfirmasi_pengajuan(id_pengajuan):  # <-- ganti nama
    data = request.get_json()
...
# ==================== ENDPOINT LAPORAN KEUANGAN (JSON) ====================
...
def get_laporan_keuangan(id_program):
    ...
    # Ambil transaksi yang sudah valid
...
def upload_user_photo(id_user):
    ...
    if file and allowed_file(file.filename):
        ...
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        file.save(filepath)
        # simpan path ke database user (misal tambah kolom foto)
        user = User.query.get(id_user)
        if user:
            user.foto = filename  # asumsi ada kolom foto
            db.session.commit()
...
def get_rab_by_id(id_rab):
    """Mengambil satu data RAB berdasarkan ID"""
...
def create_rab():
    """Menambah data RAB baru"""
    data = request.get_json()
    
    # Validasi
    if not data.get('id_program'):
        return jsonify({'status': 'error', 'message': 'ID Program harus diisi'}), 400
    if not data.get('nama_item'):
        return jsonify({'status': 'error', 'message': 'Nama item harus diisi'}), 400
    
    # Cek apakah program ada
    program = ProgramKerja.query.get(data['id_program'])
    if not program:
        return jsonify({'status': 'error', 'message': 'Program tidak ditemukan'}), 404
    ...
    return jsonify({
        ...
        'data': {
            'id_rab': new_rab.id_rab,
            'id_program': new_rab.id_program,
            'nama_item': new_rab.nama_item
        }
    }), 201
...
def update_rab(id_rab):
    ...
    return jsonify({
        'status': 'success',
        'message': 'Item RAB berhasil diupdate'
    })
...
def get_rab_by_program(id_program):
    """Mengambil semua RAB berdasarkan program tertentu"""
...
def get_tahun_list():
    """Mengambil daftar semua tahun yang tersedia"""
...
def tambah_tahun():
    ...
    if not existing:
        new_periode = PeriodeAktif(
            nama_periode=f'Periode {tahun}',
            tahun=tahun,
    ...
    return jsonify({'status': 'success', 'message': 'Tahun berhasil ditambahkan'})


@app.route('/api/periode/tahun/<path:tahun>', methods=['DELETE'])
def hapus_tahun(tahun):
    """Menghapus tahun dari daftar"""
    periode = PeriodeAktif.query.filter_by(tahun=tahun).first()
    if periode:
        db.session.delete(periode)
        db.session.commit()
    
    return jsonify({'status': 'success', 'message': 'Tahun berhasil dihapus'})


@app.route('/api/periode/aktif', methods=['GET'])
def get_periode_aktif_list():
    """Mengambil daftar periode yang aktif"""
    periode_aktif = PeriodeAktif.query.filter_by(status_periode='Aktif').all()
...
def set_periode_aktif():
    """Menyimpan periode aktif (maksimal 2)"""
    data = request.get_json()
    ...
    return jsonify({
        'status': 'success', 
        'message': 'Periode aktif berhasil disimpan',
        'data': tahun_list
    })

# ==================== ENDPOINT RIWAYAT (PAGINATION) ====================

@app.route('/api/riwayat', methods=['GET'])
def get_riwayat_pagination():
    """Mengambil riwayat program kerja dengan pagination"""
    page = request.args.get('page', 1, type=int)
    limit = request.args.get('limit', 10, type=int)
    tahun = request.args.get('tahun', None)
    
    # Query dasar: program yang sudah selesai
    ...
    # Urutkan: periode terbaru dulu, lalu created_at terbaru
